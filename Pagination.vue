<template>
  <div class="card-footer d-flex align-items-center">
    <p class="m-0 text-muted">
      ...
      <span>{{
        isLastPage ? total : Number(currentPage) * Number(limit)
      }}</span>
      of <span>{{ options.total }}</span> entries
    </p>
    <ul class="pagination m-0 ml-auto">
      ...
      <li class="d-flex" v-if="pages.length > 20">
        <span
          class="page-item"
          :class="{ active: page.pageNumber === getCurrentPage }"
          v-for="page in pages.slice(
            getPageRange.lowerBound,
            getPageRange.upperBound
          )"
          :key="page.pageNumber"
        >
          <router-link
    ...
    next() {
      return `${this.getPath}?offset=${
        this.$route.query.offset
          ? Number(this.$route.query.offset) + 1
          : this.offset + 1
      }&limit=${this.limit}&page=${Number(this.currentPage) + 1}`;
    },
    nextOffset() {
